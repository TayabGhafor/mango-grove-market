...
- 2026-04-10: Improved admin dashboard error reporting (wrap Supabase errors as real Error instances) and added a schema reconciliation migration for cases where only `profiles` was created. Verified: admin build passes and dashboard now shows the real failing table/policy instead of a generic message.
